class Graph:
    ...
    def get_unvisited_neighbors(self, node: str, route):
        ...
        small_caves_visits = [route.count(c) for c in route if c.islower()]
        double_visit = True if any([visit > 1 for visit in small_caves_visits]) else False
        if double_visit:
            neighbors = [n for n in neighbors if n.isupper() or n not in route]
        else:
            neighbors = [n for n in neighbors if n != 'start']
        return neighbors
